...
static void hydro_kx_encrypt(hydro_kx_state *state, uint8_t *c,
    const uint8_t *m, size_t mlen, const uint8_t psk[hydro_kx_PSKBYTES])
{
...
static int hydro_kx_scalarmult(hydro_kx_state *state,
...
static void hydro_kx_final(hydro_kx_state *state,
    uint8_t rx[hydro_kx_SESSIONKEYBYTES], uint8_t tx[hydro_kx_SESSIONKEYBYTES])
{
    ...
    hydro_kdf_derive_from_key(
        rx, hydro_kx_SESSIONKEYBYTES, 0, hydro_kx_CONTEXT, state->ck);
    hydro_kdf_derive_from_key(
        tx, hydro_kx_SESSIONKEYBYTES, 1, hydro_kx_CONTEXT, state->ck);
    ...
    const uint8_t psk[hydro_kx_PSKBYTES], const hydro_kx_keypair *static_kp)
{
    ...
    hydro_hash_hash(state->h, sizeof state->h, state->eph_kp.pk,
        sizeof state->eph_kp.pk, hydro_kx_CONTEXT, state->h, sizeof state->h);
    ...
    const uint8_t psk[hydro_kx_PSKBYTES], const hydro_kx_keypair *static_kp)
{
    ...
    hydro_hash_hash(state->h, sizeof state->h, peer_eph_pk,
        hydro_kx_PUBLICKEYBYTES, hydro_kx_CONTEXT, state->h, sizeof state->h);

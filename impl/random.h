...
hydro_random_init(void)
{
    ...
    //  This constant depends of mbedTLS configuration (whether the PRNG is backed by SHA256/SHA512 at this time)
    ...
    do
    {
        ...
        //Forces mbedTLS to fetch fresh entropy
        mbedtls_entropy_gather( &entropy );
        if(mbedtls_entropy_func( &entropy, 
                                &hydro_random_context.state[pos],
                                currentChunkSize ) != 0)
            return -1;
